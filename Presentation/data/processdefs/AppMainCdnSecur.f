...
#                        a Owner has change.
# D02599 14mar2012 CRICHARD	Attribution du numéro de contrat par le système	
# J03226  25Apr2013 JFO   Sur ajout de propriétaire ne pas rendre la date de naissance obligatoire.	
...
# J03693  21Feb2014 JFO  Remettre le champ MIR-CLI-ADDR-TYP-CD à blanc une fois l'adresse de l'assuré principal trouvée.
# J03395  24Feb2014 JFO   Add Title for Payor search/List/Update/Create by Application screen.
# J03978  27Jul2015 JFO  Ajouter une validation entre la première page de saisie vs la table GAAP (Gestion d'activité (Proposiiton).
#                        Ajouter le numéro du ID Témoin à l'écran de saisie.
#                        Laisser entrer en paramètre la valeur des variables  MIR-GDT-OBJ-ID & MIR-POL-APP-FORM-ID 
...
# J04714 18Mar2016  JFO  Passer en paramètre "SYSTEM" -> MIR-TRNXT-RPT-DSTRB-CD lors de l'appel du P-STEP BF0214-P
#						 pour l'impression du contrat en PDF. Cette manière l'impression se fait au 20 minutes.
# J05204  02Feb2017 JFO  Ajouter la recherche pour client signataires autorisés.
# J05311  06Jun2017 JFO  changement obligatoire et conformité.
# J00825  05Aou2019 JPA  Initialiser certains champs pour assurer que l'impression s'exécute
#-----------------------------------------------------------------------------
...
#J03245 --------------- END OF CHANGE ------------
	...
	# *****************************************************************
	# RM202A SÉCURIGROUPE : Ajouter l'écran pour visualiser couvertures existantes
	# *****************************************************************
...
#D02599 fin
	...
	# *****************************************************************************************
	# RM202A SÉCURIGROUPE : Pour Sécurigroupe, on retourne à l'écran des couvertures du client.
	# *****************************************************************************************
	IF action == "ACTION_PREVIOUS"
	{
...
#J03395 Mettre en commentaire le passage de paramètre suivant. Pas nécessaire
...
#---- J05204 FAIRE UNE RECHERCHE POUR LE NOM DU SIGNATAIRE AUTORISÉ PERSON CIE
	#J05311 BEGIN. SI L'OPTION CHOISI À L'ÉCRAN EST "même que l'assuré 1" on doit forcer la valeur du premier assuré aux 
	# 		variables MIR-DV-APC-CLI-ID-T[1] ET MIR-DV-APC-CLI-NM-T[1] AFIN QU'ILS SOIENT RETENUES EN MÉMOIRE
			...
			IF MIR-DV-CALL-FROM-GAAP-IND == "Y"
			{
				...
				IF UPPER(ws-gdt-obj-id) != UPPER(MIR-GDT-OBJ-ID)
				{
					STEP SendUserMessage01
					{
						...
						# FATAL ERROR - L'ID témoin @1 n'est pas relié à la proposition @2 sur Gestion d'acitvité (Proposition) 
				...
				IF ws-pol-id != ""
				{
					STEP SendUserMessage02
					{
						...
						# FATAL ERROR - Le numéro de police @1 est déjà relié à la propositon @2 sur gestion d'actitvité (Proposition) 
...
#					# FATAL ERROR - Proposition électronique doit être sélectionné si numéro de proposition débute par 'E'
...
#					# FATAL ERROR - Si Proposition électronique est sélectionnée, le numéro de proposition doit débuter par 'E'
...
#				BRANCH ApplicationDataInput;
#			}				
#J03978 Si toutes les validation préceédente sont OK on peut générer et créer le nouveau numéro de police
			STEP PolicyCreate
			{
...
# RM202A Remplacer les steps Health ou traditionnelles pour un seul step Sécurigroupe
...
#J03395 END
	...
	#IN003  Réactivate STEP InsuredData for LifeSuite in put BRANCH in comment out

	#RM202A Désactivater STEP InsuredData for LifeSuite in put BRANCH in comment out
	...
	# MD007K ANO-212 MIR-POL-MIB-SIGN-CD doit être initialisé seulement s'il est vide   
...
#J03558 End
	...
	# RM202A Debut  (Mettre le contrat Sécurigroupe en vigueur une fois la saisie complétée)
...
#					{
#						USES P-STEP "BF9261-P";
#						# Le PDF du contrat (@1) n'a pu être envoyé automatiquement vers la SGED
...
#					{
#						USES P-STEP "BF9261-P";
#						# Le PDF du contrat (@1) a été envoyé automatiquement vers la SGED
